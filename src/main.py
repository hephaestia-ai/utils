...
def load_files(vector_id):
    ...
    file_manager.batch_upload(vector_id, loaded_files=search_files('src', '.py'))
...
def main():
    ...
    vector_id_parser = subparsers.add_parser('get-vector-id', help='Get vector id for files to be uploaded to')
    vector_id_parser.add_argument('store_name', type=str, help='Pass the vector name / store name to get id')
    ...
    load_files_parser = subparsers.add_parser('load-files', help='Load files to specific vector')
    load_files_parser.add_argument('vector_id', type=str, help='Vector ID for file upload')

    load_file_parser = subparsers.add_parser('load-file', help='Load a singular file')
    load_file_parser.add_argument('file_name', type=str, help='Full name of the file to upload')
    load_file_parser.add_argument('purpose', type=str, help='Purpose of the file upload')

    load_status_parser = subparsers.add_parser('get-load-status', help='Get the load status of files in a vector')
    load_status_parser.add_argument('vector_id', type=str, help='Vector ID to get load status')
