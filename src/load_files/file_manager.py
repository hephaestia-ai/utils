...
class FileManager(Client):
    ...
    def delete_all(self, loaded_files):
        ...
        confirmation = input("Are you sure you want to delete all files? (y/N): ").strip().lower()
        if confirmation == 'y':
            for record in loaded_files: 
                self.client.files.delete(record) # API request # TBD Pass on template?
    ...
    def upload(self, file_name, purpose): 
        ...
        """
        return self.client.files.create(file=open(file_name, "rb"), purpose=purpose)
